...
class BertSelfAttention(nn.Module):
  def __init__(self, config):
    ...
    self.all_head_size = self.num_attention_heads * self.attention_head_size # hidden_size = all_head_size
  ...
  def transform(self, x, linear_layer):
    ...
    proj = proj.transpose(1, 2) # [bs, num_attention_heads, seq_len, attention_head_size]
    return proj
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    attn_output_weights = (torch.einsum("blxd,blyd->blxy", query, key) * scale) + attention_mask # [bs, num_attention_heads, seq_len, seq_len]
    # normalize the scores along the last dimension 
    attn_output_weights = attn_output_weights.softmax(dim=-1) # [bs, num_attention_heads, seq_len, seq_len]
    attn_output_weights = self.dropout(attn_output_weights)

    # multiply the attention scores to the value and get back V' 
    attn_output = torch.einsum("blxy,blyd->blxd", attn_output_weights, value) # [bs, num_attention_heads, seq_len, attention_head_size]
    
    # rearange the output to [bs, seq_len, num_attention_heads, attention_head_size]
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    attn_output = self.add_norm(hidden_states, attn_value, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def forward(self, input_ids, attention_mask):
    ...
    first_tk = sequence_output[:, 0] # the first token is the [CLS] token, get the embedding/representation of it
